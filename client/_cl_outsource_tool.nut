...
global function ClientCodeCallback_ToggleOutsourceBaseModel
...
{
	asset baseModel = $"mdl/dev/empty_model.rmdl"
	...
	if ( file.showBaseSkin )
	{
		...
		{
			case eAssetType.ASSETTYPE_CHARACTER:
				baseModel = CharacterSkin_GetBodyModel( GetDefaultItemFlavorForLoadoutSlot( EHI_null, Loadout_CharacterSkin( file.availableAssets[ file.currentAsset ] ) ) )
				break

			case eAssetType.ASSETTYPE_WEAPON:
				ItemFlavor skin = GetDefaultItemFlavorForLoadoutSlot( EHI_null, Loadout_WeaponSkin( file.availableAssets[ file.currentAsset ] ) )
				if ( file.useWorldModel )
					baseModel = WeaponSkin_GetWorldModel( skin )
				else
					baseModel = WeaponSkin_GetViewModel( skin )

				break
		}
	}

	file.viewerBaseSkinModel.SetModel( baseModel )
}
...
void function ServerCallback_OVSpawnModel( vector pos, vector ang )
{
	...
	file.viewerBaseSkinModel = CreateClientSidePropDynamic( pos, ang, $"mdl/dev/empty_model.rmdl" )
	...
	file.viewerBaseSkinModel.SetParent( file.viewerModel )
...
void function ClientCodeCallback_ToggleOutsourceBaseModel( bool enableBaseModel )
{
#if DEV && PC_PROG
