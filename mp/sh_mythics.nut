	...
	foreach ( var skinBlock in IterateSettingsArray( skinDataArray ) )
	{
		...
		if ( tierIdx == 1 )
		{
			...
			if ( skydivetrailAsset != $"" )
				fileLevel.mythicSkinsGUIDToCustomSkydivetrailGUID[ preRegGUID ] <- GetUniqueIdForSettingsAsset( skydivetrailAsset )
		}
...
bool function Mythics_GetIsExecutionUsableOnTier1AndTier2( ItemFlavor item )
{
	ItemFlavor character = Mythics_GetCharacterForSkin( item )
	int characterGUID = ItemFlavor_GetGUID( character )
	if ( characterGUID in fileLevel.charactersGUIDToMythicSkinGUIDs )
	{
		int skinGUID = ItemFlavor_GetGUID( item )
		if ( fileLevel.charactersGUIDToMythicSkinGUIDs[ characterGUID ].contains( skinGUID ) )
			return fileLevel.mythicCharactersGUIDToExecutionUsableOnTier1And2[ ItemFlavor_GetGUID( character ) ]
	}
...
{
	if ( Mythics_GetIsExecutionUsableOnTier1AndTier2( skin ) )
...
ItemFlavor function Mythics_GetCustomExecutionForCharacterOrSkin( ItemFlavor item )
{
	...
	if ( ItemFlavor_GetType( item ) == eItemType.character_skin && Mythics_GetIsExecutionUsableOnTier1AndTier2( item ) )
		item = Mythics_GetCharacterForSkin( item )
