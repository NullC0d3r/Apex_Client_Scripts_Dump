...
void function InitSystemMenu( var newMenuArg )                                               
{
	...
	file.qaFooter = AddMenuFooterOption( menu, LEFT, BUTTON_X, true, "#X_BUTTON_QA", "QA", ToggleOptIn, ShouldDisplayOptInOptions )
		...
		AddMenuFooterOption( menu, LEFT, BUTTON_STICK_LEFT, true, "#BUTTON_VIEW_WELCOME_TRAILER", "#VIEW_WELCOME_TRAILER", ViewWelcomeCinematic, IsLobby )
	#else
...
void function UpdateSystemMenu()
{
	...
	if ( IsConnected() && !IsLobby() )
	{
		...
		int gameState = GetGameState()
		{
			...
			else if( !MenuStack_Contains( GetMenu( "CharacterSelectMenuNew" ) ) )
		...
		if ( IsPrivateMatch() && HasMatchAdminRole() && playingOrSuddenDeath )
			...
			if ( Control_IsModeEnabled() && gameState == eGameState.Playing && GetTeam() != 0 && GetTeam() != 1 )
	...
	else
	{
		...
		#endif
		if ( IsPrivateMatchLobby() && !MenuStack_Contains( GetMenu( "CharacterSelectMenuNew" ) ))
			SetButtonData( buttonIndex++, file.leaveMatchButtonData )
	}
