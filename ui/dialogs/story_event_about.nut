...
void function OnOpen()
{
	...
	Assert( IsItemFlavorStructValid( event.guid, eValidation.DONT_ASSERT ), "Must call StoryEventAboutDialog_SetEvent before opening the StoryEventAboutDialog menu!" )
...
bool function IsChapterTimeLocked( int idx )
{
	if(file.challengeGroups.len() > idx)
	{
		...
		if ( requiredStartDateUnixTimeOrNull == null )
			Assert( false, format( "Bad format in playlist for setting: '%s'", requiredStartDateUnixTimeOrNull ) )
...
void function RVButton_OnActivate( var btn )
{
	...
	settings.video = data.radioVignette
	settings.milesAudio = data.radioVignetteMilesEvent
	settings.videoCompleteFunc = Vignette_OnVideoComplete
...
void function AddStoryTrackerPinEvent( int panelIndex, int previousPanelIndex )
{
	...
	string eventName = ItemFlavor_GetHumanReadableRefForPIN_Slow(file.event);
