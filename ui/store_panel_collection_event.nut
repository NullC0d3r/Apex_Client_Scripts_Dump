...
void function CollectionEventPanel_Init( var panel )
{
	...
	file.giftButton  = Hud_GetChild( panel, "Purchase1PackButton" )

	file.aboutButtonInvis = Hud_GetChild( panel, "AboutButtonInvisible" )
	int giftsLeft = Gifting_GetRemainingDailyGifts()
	HudElem_SetRuiArg( file.giftButton, "buttonDescText", Localize( "#GIFTS_LEFT_FRACTION", giftsLeft ) )

	file.purchaseButton      = Hud_GetChild( panel, "PurchaseNPacksButton" )
	...
	HudElem_SetRuiArg( file.giftButton, "buttonText", "#BUY_GIFT_STAR" )
	HudElem_SetRuiArg( file.purchaseButton, "buttonText", "#PURCHASE" )
	HudElem_SetRuiArg( file.bigInfoBox, "priceText1", Localize( "#LOOT_TICK_PRICE_1", 700 ) )
	...
	Hud_AddEventHandler( file.aboutButtonInvis, UIE_CLICK, AdvanceMenuEventHandler( GetMenu( "CollectionEventAboutPage" ) ) )

	Hud_AddEventHandler( file.purchaseButton, UIE_CLICK, void function( var button ) {
		PurchasePackButton_OnClick( button, 1 )
	} )
	Hud_AddEventHandler( file.giftButton, UIE_CLICK, GiftButton_Activate )

	Hud_AddEventHandler( file.giftButton, UIE_GET_FOCUS, RewardButton_OnGetFocus )
	Hud_AddEventHandler( file.giftButton, UIE_LOSE_FOCUS, RewardButton_OnLoseFocus )
	Hud_AddEventHandler( file.purchaseButton, UIE_GET_FOCUS, RewardButton_OnGetFocus )
	Hud_AddEventHandler( file.purchaseButton, UIE_LOSE_FOCUS, RewardButton_OnLoseFocus )
	...
	AddPanelEventHandler_FocusChanged( panel, CollectionEvent_OnFocusChanged )
...
void function CollectionEventPanel_UpdateGRXDependantElements()
{
	...
	if ( haveActiveCollectionEvent )
	{
		...
		{
			Hud_SetNavDown( file.aboutButton, file.completionRewardBox )
			Hud_SetNavUp( file.aboutButton, null )
		}
		...
		{
			ItemFlavorBag basePrice = expect array<GRXScriptOffer>( CollectionEvent_GetPackOffers( activeCollectionEvent ) )[0].prices[0]
			HudElem_SetRuiArg( file.bigInfoBox, "priceText1", Localize( "#LOOT_TICK_PRICE_1", GRX_GetFormattedPrice( basePrice, 1 ) ) )
		}
...
void function CollectionEvent_OnFocusChanged( var panel, var oldFocus, var newFocus )
{
	if ( !IsValid( panel ) )
		return
	...
	if( !Hud_IsFocused( file.aboutButton ) )
	{
		if ( file.aboutButton != null && file.aboutButtonInvis != null )
			HudElem_SetRuiArg( file.aboutButton , "isFocused", Hud_IsFocused( file.aboutButtonInvis ) )
	}
}
...
void function UpdateFocusStuff( var focusedRewardButtonOrNull )
{
	...
	else if ( focusedRewardButtonOrNull == file.giftButton || focusedRewardButtonOrNull == file.purchaseButton )
	{
		if ( !GRX_IsOfferRestricted() )
		{
			focusFlav = CollectionEvent_GetMainPackFlav( activeCollectionEvent )
	...
	else if ( isCompletionReward )
	{
		...
		if ( menuHeirloomOverrideGUID >= 0 )
			focusFlav = GetItemFlavorByGUID( menuHeirloomOverrideGUID )
#endif       
...
void function PurchasePackButton_OnClick( var btn, int count )
{
	...
	pdc.quantity = count
	pdc.markAsNew = false
	GRXScriptOffer offer = 	expect array<GRXScriptOffer>( CollectionEvent_GetPackOffers( activeCollectionEvent ) )[0]
	pdc.offer = offer
	PurchaseDialog( pdc )
}
...
void function GiftButton_Activate( var button )
{
	...
	ItemFlavor activeCollectionEvent = expect ItemFlavor(file.activeCollectionEvent)
	array<GRXScriptOffer> ornull offers =  CollectionEvent_GetPackOffers( activeCollectionEvent )
	if ( offers == null )
	{
		EmitUISound( "menu_deny" )
		return
