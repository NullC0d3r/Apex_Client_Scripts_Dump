...
global function JumpToMythicOfferScreen
...
void function JumpToMythicOfferScreen( string legendLink, int mythicType )
{
	JumpToHeirloomShop()
	array<GRXScriptOffer> offers
	if ( mythicType == eMythicType.PRESTIGE_SKIN )
	{
		file.mythicSkinsEnabled = true
		offers = GRX_GetLocationOffers( "prestige_set_shop" )
	}
	else if ( mythicType == eMythicType.HEIRLOOM )
	{
		file.mythicSkinsEnabled = false
		offers = GRX_GetLocationOffers( "heirloom_set_shop" )
	}
	else
	{
	...
	if ( legendLink.len() == 0 )
		return
	string legendNameUpper = legendLink.toupper()
	GRXScriptOffer matchedOffer
	foreach( offer in offers )
	{
		array<string> splitOfferTitleText = split( offer.titleText, "_#" )
		if ( splitOfferTitleText.len() == 0 )
			continue
		string legendNameInGRXOffer = splitOfferTitleText[0]
		if ( legendNameUpper == legendNameInGRXOffer )
		{
			matchedOffer = offer
			break
		}
	}
	if ( matchedOffer.items.len() == 0 )
		return
	if ( mythicType == eMythicType.PRESTIGE_SKIN )
		StoreMythicInspectMenu_AttemptOpenWithOffer( matchedOffer )
	else
		StoreInspectMenu_AttemptOpenWithOffer( matchedOffer )
}
