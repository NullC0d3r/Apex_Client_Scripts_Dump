...
void function Control_OnPlayerSentSpawnRequest( entity player, entity wp )
{
	...
	#if DEV
		printt( "CONTROL: Sending spawn request from " + wp.GetTargetName() + " of type " + respawnChoice )
	#endif       
...
void function ProcessSpawnMenu( entity player )
{
	...
	#if DEV
		printt( "CONTROL: kicking off process spawn menu thread" )
	#endif       
...
void function UICallback_Control_SpawnButtonClicked( int waypointEHI )
{
	...
	#if DEV
		if ( waypoint == null )
			printt( "CONTROL: tried to spawn on button with a waypoint that no longer exists" )
	#endif       
...
bool function Control_HandleKeyInput( int key )
{
	...
	switch ( key )
	{
		...
		case BUTTON_DPAD_UP:
		case KEY_F2:
			RunUIScript( "UI_OpenFeatureTutorialDialog" )
			isSuccessful = true
			break
		default:
