	...
	if ( player == GetLocalClientPlayer() && player == GetLocalViewPlayer() )
		...
		if ( slingPassiveRui != null )
		{
			if( SURVIVAL_GetPrimaryWeapons( player ).len() <= 0 )
				RuiSetBool( slingPassiveRui, "hasMainWeapon", false )

			RuiSetBool( slingPassiveRui, "hasSlingWeapon", !IsPlayerWeaponSlingEmpty( player ) )
		}
...
bool function Survival_DropInventoryItem( string ref, int num )
{
	...
	if ( IsValid( deathbox ) )
	{
		Remote_ServerCallFunction( "ClientCallback_Sur_DropBackpackItem_ToDeathbox", data.index, num, deathbox )
	}
	else
	{
...
void function UICallback_BackpackClosed()
{
	...
	if ( file.currentGroundListData.listType == eGroundListType.GRABBER && IsValid( oldDeathBoxEnt ) )
	{
		...
		if ( file.currentGroundListData.listType == eGroundListType.VENDINGMACHINE && IsValid( oldDeathBoxEnt ) )
		{
...
void function SurvivalMenu_Internal( entity player, string uiScriptFuncName, entity deathBox = null, int groundListBehavior = eGroundListBehavior.CONTENTS, int groundListType = eGroundListType.DEATH_BOX )
{
	...
	if ( IsEventFinale() && !IsValid(deathBox) )
		return
...
void function EquipmentButtonInit( var button, string equipmentSlot, LootData lootData, int count )
{
      ...
      
	{
		...
		if( SURVIVAL_Loot_IsRefValid( ammoTypeRef ) && weapon.GetWeaponSettingBool( eWeaponVar.uses_ammo_pool ) )
		{
